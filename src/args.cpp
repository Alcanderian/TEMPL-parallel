...
void IOParser::copy(const IOParser &in) {
    ...
    default_values = in.default_values;
    argc = in.argc;
    argv = in.argv;
...
void IOParser::addOption(std::string option, std::string usage, std::string default_value, bool hidden) {
    if (hidden)
        hidden_options.push_back(option);
    else {
        ...
        if (default_value == "NULL") {
            ...
            default_values.push_back((std::string) default_value);
        }
    }
}
...
bool IOParser::optionExists(std::string option) {
    ...
    for (int ii = 0; ii < hidden_options.size(); ii++)
        if (strcmp((hidden_options[ii]).c_str(), option.c_str()) == 0)
            return true;

    return false;
}

std::string IOParser::getOption(std::string option, std::string usage, std::string default_value, bool hidden) {
    // If this option did not exist yet, add it to the list
    if (!optionExists(option))
        addOption(option, usage, default_value, hidden);
...
// Checks if a boolean parameter was included the command line =============
bool IOParser::checkOption(std::string option, std::string usage, std::string default_value, bool hidden) {
    // If this option did not exist yet, add it to the list
    if (!optionExists(option))
        addOption(option, usage, default_value, hidden);
...
bool IOParser::checkForErrors(int verb) {
    ...
    if (argc == 1 || (argc == 2 && checkParameter(argc, argv, "--continue")) || checkParameter(argc, argv, "--help") ||
...
void IOParser::checkForUnknownArguments() {
    for (int i = 1; i < argc; i++) {
        ...
        if (strncmp("--", argv[i], 2) == 0) {
            ...
            int is_a_number = sscanf(argv[i], "%f", &test_val);
            if (is_a_number) {
...
void IOParser::writeUsage(std::ostream &out) {
    out << "+++ Command line arguments (with defaults for optional ones between parantheses) +++" << std::endl;
